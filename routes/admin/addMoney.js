...
const GET_ACCOUNT = 'SELECT c.AccountId FROM [_GF_CS_Accounts] AS acc JOIN [Character] AS c ON c.AccountId = acc.AccountId WHERE c.Name = @characterName;';
    ...
    if (recordset.length === 0) {
        try {
            const request = await server.db.request()
                ...
                .query(`${GET_ACCOUNTID}`);
            console.log(request);
            accountId = request.recordset[0].AccountId || [];
        }
        catch (error) {
            console.error(error);
            return res.status(500).send({success: false, error: global.translate.ERROR_IN_DATABASE});
